...
void RTC_DeInit(void)
{
    ...
    while(CW_RTC->CR1_f.WAIT);    // 等待RTC进入可读写状态
    ...
    /* 关闭RTC模块的时钟 */
    CW_SYSCTRL->APBEN2 =  0x5A5A0000 | (CW_SYSCTRL->APBEN2 & (~SYSCTRL_APBEN2_RTC_Msk));
}

/**
 * @brief RTC模块初始化
 *
 * @param 初始化RTC模块，用户需首先启动启动RTC时钟源！！！
 *        仅在上电复位时，对RTC配置，其他复位无效
 *
...
ErrorStatus RTC_Init(RTC_InitTypeDef* RTC_InitStruct)
{
    ...
    SYSCTRL_APBPeriphClk_Enable2(SYSCTRL_APB2_PERIPH_RTC, ENABLE);    //  启动RTC外设时钟，使能RTC模块    
 ...
 *
 * @param PSC1， PSC2
 * 用户应合理配置 PSC1 和 PSC2，以使 f TICKCLK = 2Hz
 */
...
void RTC_SetPrescale(uint8_t PSC1,uint32_t PSC2)
{
    ...
    CW_RTC->PSC_f.PSC1 = PSC1;
    CW_RTC->PSC_f.PSC2 = PSC2;
...
void RTC_GetAlarm(uint32_t RTC_Alarm, RTC_AlarmTypeDef* RTC_AlarmStruct)
{
    ...
    RTC_AlarmStruct->RTC_AlarmTime.H24 = CW_RTC->CR0_f.H24;
...
{
    assert_param(IS_RTC_AWTTIMER_SOURCE(RCT_AWTStruct->clockSource));

    // 1. 解锁RTC寄存器
    RTC_UNLOCK();
    
    // 2. 使能自动唤醒定时器
    CW_RTC->CR2 |= (1 << RTC_CR2_AWTEN_Pos);
    
    // 3. 配置唤醒定时器的时钟源和预分频
    CW_RTC->CR2 &= ~(RTC_CR2_AWTSRC_Msk | RTC_CR2_AWTPRS_Msk); // 清除旧设置
    CW_RTC->CR2 |= (RCT_AWTStruct->clockSource << RTC_CR2_AWTSRC_Pos) |
                   (RCT_AWTStruct->prescaler << RTC_CR2_AWTPRS_Pos);

    // 4. 配置唤醒定时器的重载值
    CW_RTC->AWTARR = RCT_AWTStruct->reloadValue;

    // 5. 使能唤醒定时器中断（如果需要）
    if (RCT_AWTStruct->enableInterrupt) 
    {
        CW_RTC->IER |= (1 << RTC_IER_AWTIMER_Pos);
    }
    else 
    {
        CW_RTC->IER &= ~(1 << RTC_IER_AWTIMER_Pos);
...
void RTC_SetClockSource(uint32_t RTC_ClockSource)
{
    ...
    CW_RTC->CR1 = RTC_ClockSource;    // 设置RTC时钟源
...
{
    assert_param(IS_RTC_AWT_PSC(RtcAwtPscValue));
    RTC_UNLOCK();
    
    CW_RTC->CR2_f.AWTPRS = RtcAwtPscValue; 
...
void RTC_SetRTCCLKD_Prescaler(uint32_t prescalerValue) 
{
    ...
    // 清除PSC1的旧值
    CW_RTC->PSC &= ~RTC_PSC_PSC1_Msk;

    // 设置新的PSC1值
    CW_RTC->PSC |= (prescalerValue << RTC_PSC_PSC1_Pos);
...
void RTC_SetTICKCLK_Prescaler(uint32_t prescalerValue) 
{
    ...
    // 清除PSC2的旧值
    CW_RTC->PSC &= ~RTC_PSC_PSC2_Msk;

    // 设置新的PSC2值
    CW_RTC->PSC |= prescalerValue;
